...
def extract_audio_with_ffmpeg(input_path: str, output_path: str) -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            text=True,
            timeout=600,
            cwd=os.path.dirname(input_path)  # Set working directory to input file location
        )
...
def transcribe_audio(
    ...
    try:
        ...
        # Transcribe with Whisper - pass file path directly
        try:
            ...
            if "'NoneType'" in error_str and "write" in error_str:
                raise RuntimeError(
                    ...
                    f"3. There's not enough disk space for temp files\n\n"
        ...
        # Clean up temp files
        for temp_file in [temp_audio_path, copied_temp_file]:
